...
python pancancer_classifier.py --genes 'TP53' --diseases $tp53_diseases --drop \
        --copy_number --remove_hyper --alt_folder $tp53_dir --alphas $alpha \
        --l1_ratios $l1_mixing

# 2. Within disease type TP53 classification
python within_tissue_analysis.py --genes 'TP53' --diseases $tp53_diseases \
        --remove_hyper --alt_folder $tp53_dir'/within_disease' --alphas $alphas \
        --l1_ratios $l1_mixing

# 3. Prepare heatmap figure describing TP53 mutations across the set
python build_heatmap_data.py --genes 'TP53' --copy_number --remove_hyper \
        --alt_folder $tp53_dir

# 4. Apply PanCan classifier to all samples and output scores for each sample
python apply_weights.py --classifier $tp53_dir --copy_number

# 5. Summarize and visualize performance of classifiers
python visualize_decisions.py --scores $tp53_dir --custom 'TP53_loss'
python map_mutation_class.py --scores $tp53_dir --genes 'TP53'
Rscript --vanilla ddr_summary_figures.R
Rscript --vanilla compare_within_models.R --within_dir $tp53_dir'/within_disease' \
