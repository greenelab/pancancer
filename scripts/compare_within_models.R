...
pancan_auroc_df <- process_classifier_summary(summary_list = pancan_list,
                                              model_type = "Pan",
                                              perf_type = "AUROC")
pancan_auprc_df <- process_classifier_summary(summary_list = pancan_list,
                                              model_type = "Pan",
                                              perf_type = "AUPRC")
...
within_disease_auprc <- data.frame()
for (file in within_disease_files) {
  ...
  dis_auprc_df <- process_classifier_summary(summary_list = disease_summary,
                                             model_type = "Within",
                                             perf_type = "AUPRC")
  within_disease_auprc <- rbind(within_disease_auprc, dis_auprc_df)
}
...
auprc_plot <- plyr::rbind.fill(within_disease_auprc, pancan_auprc_df)
auprc_plot$AUPRC <- as.numeric(paste(auprc_plot$Performance_type))
auprc_plot$Model <- factor(auprc_plot$Model, levels = c('Within', 'Pan'))
...
auprc_comparison_plot <- ggplot(auprc_plot, aes(x = Disease, y = AUPRC,
                                                fill = Model)) +
  base_comparison_theme +
  geom_bar(position = "dodge", stat = "identity") +
  scale_fill_manual(values = c("brown", "gold")) +
...
auprc_comp_fig <- file.path(pan_summary_dir, "figures", "auprc_comparison.svg")
svg(auprc_comp_fig, width = 4.2, height = 1.4)
auprc_comparison_plot
dev.off()
...
if (!is.null(alt_gene_dir)) {
  ...
  pancan_auroc_df <- process_classifier_summary(summary_list = pancan_alt_list,
                                                model_type = "Pan",
                                                perf_type = "AUROC")
  pancan_auprc_df <- process_classifier_summary(summary_list = pancan_alt_list,
                                                model_type = "Pan",
                                                perf_type = "AUPRC")
  ...
  within_disease_alt_auprc <- data.frame()
  for (file in within_alt_disease_files) {
    ...
    dis_auprc_df <- process_classifier_summary(summary_list = dis_alt_summary,
                                            model_type = "Within",
                                            perf_type = "AUPRC")
    within_disease_alt_auprc <- rbind(within_disease_alt_auprc, dis_auprc_df)
  }
  
  pancan_auroc_df <- plyr::rbind.fill(within_disease_alt_auroc, pancan_auroc_df)
  pancan_auprc_df <- plyr::rbind.fill(within_disease_alt_auprc, pancan_auprc_df)
  ...
  alt_auroc_df <- process_classifier_summary(summary_list = pancan_list,
                                             ...
                                             perf_type = "AUROC")
  alt_auprc_df <- process_classifier_summary(summary_list = pancan_list,
                                             model_type = classifier_gene,
                                             gene_type = "Alt gene performance",
                                             gene_class = "Alternative Genes",
                                             perf_type = "AUPRC")
  ...
  plot_auprc_alt$AUPRC <- as.numeric(paste(plot_auprc_alt$Performance_type))
  ...
  alt_auprc_plot <- ggplot(plot_auprc_alt, aes(x = Disease, y = AUPRC,
                                              fill = Model)) +
    geom_bar(position = "dodge", stat = "identity") +
    theme_bw() + within_theme +
    theme(legend.position = c(1.1, 0.65),
  ...
  alt_auprc_figure <- file.path(pan_summary_dir, "figures",
                                "alt_gene_auprc_comparison.svg")
  svg(alt_auprc_figure, width = 2.5, height = 1.4)
  print(alt_auprc_plot)
  dev.off()
}
